...
How to play:
	...
	~ Items will show up every so often, which offer various boosts. You collide with them to attain the boost
		...
		~ Weapon upgrades will increase the damage your weapons do
...
TLDR: The ways in which we fulfill the requirements are:
	~ All movement is either physical or parametric, with transformations
	~ 2 distinct weapons used differently
	~ 3 distinct enemies, with different behaviour
	~ Meaningful boss fight end condition with different combat mechanics to the rest of the game
	~ 3 distinct collectible items
	~ Collisions are detected using either ray-circle or circle-circle collision
	~ Map scrolls across with automated view matrix movement, and background is tiled texture
	~ 2 particle systems, one for torpedo and player trails, and one for explosions
	~ UI that shows time, health, and score
	~ Advanced method: 
		~ SHARKS use pursuit behaviour
