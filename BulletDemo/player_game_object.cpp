...
PlayerGameObject::PlayerGameObject(const glm::vec3 &position, Geometry *geom, Shader *shader, GLuint texture, float yScale, float xScale)
...
void PlayerGameObject::Update(double delta_time, glm::vec3 camera) {
	...
	if ((position_.x >= camera.x - 5.2f) && (position_.x <= camera.x + 5.2f) && (position_.y >= camera.y - 3.2f) && (position_.y <= camera.y + 3.0f)) {
		this->position_ += (float)delta_time * this->velocity_;
	}
	if (position_.x <= camera.x - 5.2f) this->position_ += glm::vec3(0.02f, 0.0f, 0.0f);
	if (position_.x >= camera.x + 5.2f) this->position_ += glm::vec3(-0.02f, 0.0f, 0.0f);
	if (position_.y <= camera.y - 3.2f) this->position_ += glm::vec3(0.0f, 0.01f, 0.0f);
